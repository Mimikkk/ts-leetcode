...
const isNice = (s: string) => {
  ...
  return ![...s].some((char) =>
    isUpperCase(char) &&
    !lowercase.has(char.toLowerCase()) ||
    isLowerCase(char) &&
    !uppercase.has(char.toUpperCase()),
  );
};
...
const substrings = (s: string) =>
  unique(
    range(0, s.length)
      .flatMap((x) => range(x, s.length)
        .flatMap((y) => s.slice(x, y + 1)),
      ),
  );

const first = (s: string[]) => s[0];
const lengthwise = (a: string, b: string) => b.length - a.length;
const longestNiceSubstring = (s: string) =>
  first(substrings(s).filter(isNice).sort(lengthwise)) ?? "";
