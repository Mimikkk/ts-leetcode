...
const inorder = (root: TreeNode | null): number[] => root ?
...
const increasingBST = (root: TreeNode | null): TreeNode | null => {
  ...
  const head = inorder(root).map(createNode).reduce((current, node) => {
...
describe("increasing bst", () => {
  it("case 1", () => {
    expect(increasingBST(node([5, 3, 6, 2, 4, null, 8, 1, null, null, null, 7, 9])))
      .toEqual(node([1, null, 2, null, 3, null, 4, null, 5, null, 6, null, 7, null, 8, null, 9]));
  });
